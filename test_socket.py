...
class AdvancedSocketTester:
    ...
    def close_socket_gracefully(self, sock, timeout=1.0):
        """优雅地关闭socket连接，避免Windows错误"""
        try:
            # 先关闭发送端
    ...
    def test_basic_tcp_server(self, host='localhost', port=8888):
        ...
        try:
            ...
            for cmd in commands:
                ...
                time.sleep(0.5)  # 减少命令间隔时间
        
            # 给服务器时间处理最后的命令
            time.sleep(0.5)
            self.close_socket_gracefully(client_socket, timeout=0.5)
    ...
    def test_concurrent_connections(self, host='localhost', port=8888, num_clients=5):
        ...
        for i in range(num_clients):
            ...
            thread.start()
            time.sleep(0.2)  # 增加连接间隔时间，减少并发压力
